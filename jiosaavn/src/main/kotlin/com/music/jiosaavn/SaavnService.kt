...
@Serializable
private data class WrapperSearchResponse(
    @SerialName("success") val success: Boolean = false,
    @SerialName("data")    val data:    WrapperSearchData? = null
)

@Serializable
private data class WrapperSearchData(
    @SerialName("total")   val total: Int = 0,
    @SerialName("results") val results: List<SaavnSong> = emptyList()
)

@Serializable
private data class WrapperSongResponse(
    @SerialName("success") val success: Boolean = false,
...
object SaavnService {
    ...
    suspend fun searchSongs(query: String): Result<List<SaavnSong>> {
        ...
        if (directResult.isSuccess) {
            ...
            Log.e(TAG, "searchSongs: direct API failed, trying wrappers for query=\"$query\"", directResult.exceptionOrNull())
            return searchSongsWithWrappers(query)
        }
    }

    private suspend fun searchSongsWithWrappers(query: String): Result<List<SaavnSong>> {
        for (i in EndpointManager.SERVERS.indices) {
            val server = EndpointManager.getCurrentServer()
            try {
                // For wrappers, we create a new request overriding the default BASE_URL
                val response = client.get("$server/api/search/songs") {
                    parameter("query", query)
                }
                if (response.status == HttpStatusCode.OK) {
                    val responseText = response.bodyAsText()
                    val body = json.decodeFromString<WrapperSearchResponse>(responseText)
                    val results = body.data?.results ?: emptyList()
                    if (results.isNotEmpty()) {
                        EndpointManager.reportSuccess(server)
                        return Result.success(results)
                    }
                }
                EndpointManager.reportFailure(server, "Empty results or non-OK status: ${response.status}")
            } catch (e: Exception) {
                EndpointManager.reportFailure(server, e.message ?: "Unknown error")
            }
        }
        return Result.failure(Exception("All Saavn fallback servers failed to search"))
    }
    ...
    suspend fun getBestStreamUrl(saavnSongId: String, quality: String): String? {
        ...
        if (result.isSuccess) {
            val directUrl = result.getOrNull()
            if (directUrl != null) return directUrl
        }
        Log.e(TAG, "getBestStreamUrl failed for saavnSongId=$saavnSongId, trying wrappers", result.exceptionOrNull())
        return getBestStreamUrlWithWrappers(saavnSongId, quality)
    }

    private suspend fun getBestStreamUrlWithWrappers(saavnSongId: String, quality: String): String? {
        for (i in EndpointManager.SERVERS.indices) {
            val server = EndpointManager.getCurrentServer()
            try {
                // For wrappers, we create a new request overriding the default BASE_URL
                val response = client.get("$server/api/songs/$saavnSongId")
                if (response.status == HttpStatusCode.OK) {
                    val responseText = response.bodyAsText()
                    val body = json.decodeFromString<WrapperSongResponse>(responseText)
                    val wrapperSong = body.data.firstOrNull()
                    if (wrapperSong != null) {
                        EndpointManager.reportSuccess(server)
                        return selectBestUrl(wrapperSong.downloadUrl, quality)
                    }
                }
                EndpointManager.reportFailure(server, "Empty results or non-OK status: ${response.status}")
            } catch (e: Exception) {
