...
import com.snuggle.music.utils.BotDetectionMitigator
...
import com.snuggle.music.utils.YTPlayerUtils.MAIN_CLIENT
import com.snuggle.music.utils.YTPlayerUtils.STREAM_FALLBACK_CLIENTS
import com.snuggle.music.utils.YTPlayerUtils.validateStatus
import com.snuggle.music.utils.potoken.PoTokenGenerator
import com.snuggle.music.utils.potoken.PoTokenResult
import com.snuggle.music.utils.sabr.EjsNTransformSolver
        ...
        suspend fun trySaavn(): Result<PlaybackData> {
            ...
            try {
                saavnAttempt = kotlinx.coroutines.withTimeoutOrNull(15000L) {
                    ...
                    if (title.isBlank()) {
                        val exc = Exception("Title is blank")
                        Timber.tag(TAG).e(exc, "trySaavn failed early: Title is blank")
                        throw exc
                    }
                    ...
                    Timber.tag(TAG).d("Saavn search result count: ${songs?.size ?: 0}")
                    if (songs.isNullOrEmpty()) {
                        val exc = Exception("Saavn: no results for \"$query\"")
                        Timber.tag(TAG).e(exc, "trySaavn search failed: no results")
                        throw exc
                    }

                    val ytDuration = knownDurationMs?.let { it / 1000L } ?: metadata?.videoDetails?.lengthSeconds?.toLongOrNull() ?: 0L
                    Timber.tag(TAG).d("YouTube metadata: title=\"$title\", artist=\"$artist\", duration=${ytDuration}s")
                    ...
                    val scored = songs.map { candidate ->
                        val titleScore = wordOverlapScore(title, candidate.name, maxPts = 50)
                        
                        val saavnDuration = candidate.duration?.toLong() ?: 0L
                        val durationScore = if (ytDuration > 0 && saavnDuration > 0) {
                        ...
                        val explicitScore = if (candidate.explicitContent) 5 else 0
                        
                        val finalScore = titleScore + durationScore + artistScore + explicitScore
                        
                        Timber.tag(TAG).d("Saavn Candidate: title=\"${candidate.name}\", artist=\"$saavnArtists\", duration=${saavnDuration}s | Sub-scores: title=$titleScore, artist=$artistScore, duration=$durationScore, explicit=$explicitScore | Final Score: $finalScore")
                        
                        ScoredSong(candidate, finalScore)
                    }

                    val MIN_CONFIDENCE = 40
                    val bestScored = scored.maxByOrNull { it.score }
                    Timber.tag(TAG).d("MIN_CONFIDENCE threshold: $MIN_CONFIDENCE")
                    
                    val bestSong = bestScored?.takeIf { it.score >= MIN_CONFIDENCE }?.song

                    if (bestSong == null) {
                        Timber.tag(TAG).d("No candidate reached MIN_CONFIDENCE! Highest scoring candidate: name=\"${bestScored?.song?.name}\", id=${bestScored?.song?.id}, score=${bestScored?.score ?: "none"}")
                        val exc = Exception("Saavn: best score below threshold $MIN_CONFIDENCE (score=${bestScored?.score ?: "none"})")
                        Timber.tag(TAG).e(exc, "trySaavn confidence check failed")
                        throw exc
                    }

                    Timber.tag(TAG).d("Selected Saavn candidate: id=${bestSong.id}, name=\"${bestSong.name}\" (score=${bestScored?.score})")

                    val streamUrl = com.music.jiosaavn.SaavnService.getBestStreamUrl(bestSong.id, "320kbps")
                    Timber.tag(TAG).d("Saavn stream url fetched: $streamUrl")
                    if (streamUrl.isNullOrBlank()) {
                        val exc = Exception("Saavn: no stream URL for songId=${bestSong.id}")
                        Timber.tag(TAG).e(exc, "trySaavn stream URL lookup failed")
                        throw exc
                    }
                ...
                if (saavnAttempt == null) {
                    ...
                    Timber.tag(TAG).e(lastException, "trySaavn attempt failed due to timeout")
                }
            } catch (e: Exception) {
                lastException = e
                Timber.tag(TAG).e(e, "trySaavn failed with exception")
            }
...
fun confidence(queryArtist: String, queryTitle: String, queryDuration: Long?, candidate: com.snuggle.music.utils.qobuz.QobuzTrack): Float {
