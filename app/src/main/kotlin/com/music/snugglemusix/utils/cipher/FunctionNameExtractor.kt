...
object FunctionNameExtractor {
    private const val TAG = "Snuggle_CipherFnExtract"
            ...
            if (match != null) {
                when (index) {
                    0 -> {
                        ...
                        Timber.tag(TAG).d("N-FUNCTION FOUND via pattern $index: name=$name, arrayIndex=$arrayIdx")
                    ...
                    1 -> {
                        ...
                        val name = match.groupValues[1]
                        Timber.tag(TAG).d("N-FUNCTION FOUND via pattern $index: name=$name")
